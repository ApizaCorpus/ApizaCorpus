...
int main(int argc, char **argv){
    ...
    ALLEGRO_SAMPLE *game_over_sample;
    ALLEGRO_FONT *font;
    ...
    if(!al_init_font_addon() || !al_init_ttf_addon()) {
        fprintf(stderr, "failed to initialize font sub-system!\n");
        return -1;
    }

    font = al_load_ttf_font(FONT_PATH, 16, ALLEGRO_TTF_MONOCHROME);
    if(!font) {
    ...
    if(!al_init_acodec_addon()) {
        fprintf(stderr, "failed to setup audio codecs!\n");
        return -1;
    }

    if(!al_reserve_samples(1)) {
        fprintf(stderr, "failed to reserve samples for the default mixer!\n");
        return -1;
    }

    game_over_sample = al_load_sample(AUDIO_SAMPLE_PATH);
    if(!game_over_sample) {
	...
	while(!doexit) {
        ...
        keys[KEY_UP] = al_key_down(&keyboard_state, ALLEGRO_KEY_UP);
        keys[KEY_DOWN] = al_key_down(&keyboard_state, ALLEGRO_KEY_DOWN);
        keys[KEY_LEFT] = al_key_down(&keyboard_state, ALLEGRO_KEY_LEFT);
        keys[KEY_RIGHT] = al_key_down(&keyboard_state, ALLEGRO_KEY_RIGHT);
		...
		/*** Display close event ***/
	  	...
	  	if(redraw && al_is_event_queue_empty(event_queue)) {
            ...
            sprintf(text_buffer, "High Score: %d", highscore);
            al_draw_text(font, 
                         al_map_rgb(100, 100, 100), 
                         640 - 100, 
                         0, 
			...
			al_flip_display();
		 	...
		 	if(checkCollision(player, enemy)) {
				al_stop_timer(timer);
                al_play_sample(game_over_sample, 1.0, 0.0, 1.5, ALLEGRO_PLAYMODE_ONCE, NULL);
				al_rest(3);
				al_start_timer(timer);
				initPlayer(&player);
				initEnemy(&enemy);
