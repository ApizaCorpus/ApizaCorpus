...
const float PAN = 0.0;
const float SPEED = 1;
const ALLEGRO_PLAYMODE PLAYMODE = ALLEGRO_PLAYMODE_ONCE;
...
int main(int argc, char **argv){
	...
	/*** Install stuff ***/
	bool keyboardSuccess = al_install_keyboard();
	if ( !keyboardSuccess ){
		printf("failed tp install keyboard\n");
		return 1;
	}
	al_register_event_source(event_queue, al_get_keyboard_event_source());

	/** audio stuff **/
	bool audioSuccess = al_install_audio();
	if( !audioSuccess ){
		fprintf(stderr, "failed to install audio!\n");
		return -1;
	}

	bool codecSuccess = al_init_acodec_addon();
	if( !codecSuccess ){
		fprintf(stderr, "failed to install audio codec!\n");
		return -1;
	}
	...
	al_load_font( FONT_PATH, int size, int flags)
	...
	while(!doexit) {
		...
		/*** Display close event ***/
	  	...
	  	/*** Update frame ***/
	  	// only redraws the frame if the event queue is empty
