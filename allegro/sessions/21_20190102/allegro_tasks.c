...
int main(int argc, char **argv){
	...
	const int scoreStringLength = 16;
	char scoreStr[scoreStringLength];
	char highscoreStr[scoreStringLength];
	...
	//al_load_bitmap(BITMAP_PLAYER_PATH);
	...
	font = al_load_ttf_font(FONT_PATH,48,0);
	...
	if (!font){
 	...
 	game_over_sfx = al_load_sample(AUDIO_SAMPLE_PATH);
	if (!game_over_sfx){
		...
		// Input
		 ...
		 switch(ev.keyboard.keycode) {
	  	...
	  	if(redraw && al_is_event_queue_empty(event_queue)) {
			...
			snprintf(scoreStr, scoreStringLength, "%d", score);
			snprintf(highscoreStr, scoreStringLength, "%d", highscore);


			al_draw_text(font, al_map_rgb(0,0,0), 0, 0, ALLEGRO_ALIGN_LEFT, scoreStr);
			al_draw_text(font, al_map_rgb(0,0,0), SCREEN_W, 0, ALLEGRO_ALIGN_RIGHT, highscoreStr);
			...
			al_flip_display();
		 	...
		 	if(checkCollision(player, enemy)) {
		 		al_play_sample(game_over_sfx, 1.0, 0.0,1.0,ALLEGRO_PLAYMODE_ONCE,NULL);
				al_stop_timer(timer);
				al_rest(3);
				al_start_timer(timer);
				initPlayer(&player);
