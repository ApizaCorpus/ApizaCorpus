...
ALLEGRO_SAMPLE *gameover_sample = NULL;
...
int main(int argc, char **argv){
	...
	ALLEGRO_BITMAP *spider_bitmap, *fly_bitmap;
	...
	if (!al_init_acodec_addon()) {
                fprintf(stderr, "failed to install acodec addon!\n");
                return -1;
	}

	al_reserve_samples(3);

	gameover_sample = al_load_sample(AUDIO_SAMPLE_PATH);

	if (!gameover_sample) {
		fprintf(stderr, "failed to load sample sound!\n");
		return -1;
	}
	...
	score_font = al_load_font(FONT_PATH, 40, 0);
	...
	spider_bitmap = al_load_bitmap(BITMAP_ENEMY_PATH);
	fly_bitmap = al_load_bitmap(BITMAP_PLAYER_PATH);

	if (!spider_bitmap || !fly_bitmap) {
	...
	al_draw_scaled_bitmap(fly_bitmap, 0, 0, 128, 128,
                                          0, 0, PLAYER_SIZE, PLAYER_SIZE, 0);


	al_set_target_bitmap(enemy_bitmap);
	//al_clear_to_color(al_map_rgb(255, 0, 0));
        al_draw_scaled_bitmap(spider_bitmap, 0, 0, 128, 128,
                                          0, 0, ENEMY_SIZE, ENEMY_SIZE, 0);
	...
	while(!doexit) {
		...
		if(ev.type == ALLEGRO_EVENT_TIMER) {
			if(ev.timer.source == timer) {
				...
				keys[KEY_UP] = al_key_down(&kstate, ALLEGRO_KEY_UP);
				keys[KEY_DOWN] = al_key_down(&kstate, ALLEGRO_KEY_DOWN);
				keys[KEY_LEFT] = al_key_down(&kstate, ALLEGRO_KEY_LEFT);
				keys[KEY_RIGHT] = al_key_down(&kstate, ALLEGRO_KEY_RIGHT);
		...
		/*** Display close event ***/
	  	...
	  	if(redraw && al_is_event_queue_empty(event_queue)) {
			...
			al_draw_text(score_font, al_map_rgb(100,100,100), 0, 0, 0, buff);

			sprintf(buff, "%d", highscore);
			al_draw_text(score_font, al_map_rgb(100,100,100), SCREEN_W, 0, ALLEGRO_ALIGN_RIGHT, buff);
			...
			al_flip_display();
		 	...
		 	if(checkCollision(player, enemy)) {
				...
				printf("playing game over sample...\n");
				al_play_sample(gameover_sample, 1, 1, 1, ALLEGRO_PLAYMODE_ONCE, &ret_id);
